...
class NERLayoutLMv2(LayoutLMv2PreTrainedModel):

    def __init__(self, config: LayoutLMv2Config, num_question, max_seq_len, init_embedding=None,
                 use_multiple_attention=True, embedding_from_encoder=False):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.layoutlmv2 = LayoutLMv2Model(config)  # we can change it to bert here
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.max_seq_len = max_seq_len
        self.use_multiple_attention = use_multiple_attention
        self.num_question = num_question
        self.num_ner_label = num_question * 4 + 1  # B-, I-, E-, S-, O
        ...
        if embedding_from_encoder:
            print("The embedding will be initialized using the encoder")
            self.question_embedding = None
        else:
            if init_embedding == None:  # if the embedding is not provided then initilize it randomly
        ...
        # self.end_projection = nn.Linear(config.hidden_size, config.hidden_size, bias=False)
        self.init_weights()
    ...
    def forward(
        ...
        # add layoutlm later
        outputs = self.layoutlmv2(**parameters)
        logits = outputs[0]
        logits = self.dropout(logits)  # bs x seq x h
        loss_fct = CrossEntropyLoss()
        ner_loss_fct = CrossEntropyLoss(ignore_index=self.num_ner_label - 1)
        # NER part
